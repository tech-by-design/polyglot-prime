...
## Follow the below instructions for a successful Winter'24 to 1115-SDOH-Template CSV schema migration: 
  ...
  1. **Renamed CSV Files**
  - To maintain the ordinality and cardinality of the data in alignment with **IG 1.3.0**, the CSV file naming conventions have been updated. The following files used in **Winter'24** have been **renamed** in **1115-SDOH-Template** while maintaining their structure and format:
...
## TechBD updates existing Winter'24 CSV transformer to include new IG v1.3.0 content.  
...
## Data from submitter in existing Winter'24 CSV column must be using IG v1.3.0 constraints (using NYHER value sets). For example:
...
## Data from submitter in new CSV column that is not in Winter'24 using IG v1.3.0 constraints (using NYHER value sets). For Example:
...
1. **Field Name Alignment:**
 - IG v1.3.0 terms and field names differed from CSV field names, so they were aligned.

2. **New CSV for Columns for all CSV Files:**
 - IG v1.3.0 requires description and system along with code, so a new CSV file was added.
