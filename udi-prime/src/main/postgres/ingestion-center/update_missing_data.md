# Update FHIR Count, CSV Validation Status, Tenant id, Bundle Type and Replay Status values in various tables
...
- `update_fhir_bundle_type.psql`
- `update_fhir_replay_status.psql`
...
- correct the `data_validation_status` column of `sat_interaction_zip_file_request` table
- fill the newly added `tenant_id` column in various tables
- update the `bundle_type` column of `sat_interaction_fhir_request` table with new logic
- back fill the `replay_status` value with the status of the replay process into `sat_interaction_fhir_request` table
...
4. The target tables should be there in the `techbd_udi_ingress` schema.
...
### Step 4: Execute the SQL File (update_tenant_ids.psql) to fill the `tenant_id` column in various tables
Run the SQL script to fill the new column:
```bash
psql -h <hostname> -U <admin_user> -d <database_name> -f update_tenant_ids.psql
```

### Step 5: Execute the SQL File (update_tenant_ids_of_http_requests.psql) to fill the `tenant_id` column in `sat_interaction_http_request` and `sat_interaction_fhir_validation_issue` tables
Run the SQL script to fill the new column:
```bash
psql -h <hostname> -U <admin_user> -d <database_name> -f update_tenant_ids_of_http_requests.psql
```

### Step 6: Execute the SQL File (update_fhir_bundle_type.psql) to update the `bundle_type` column of `sat_interaction_fhir_request` table with new logic
Run the SQL script to fill the new column:
```bash
psql -h <hostname> -U <admin_user> -d <database_name> -f update_fhir_bundle_type.psql
```

### Step 7: Execute the SQL File (update_fhir_replay_status.psql) to back fill the `replay_status` value with the status of the replay process into `sat_interaction_fhir_request` table
Run the SQL script to fill the new column:
```bash
psql -h <hostname> -U <admin_user> -d <database_name> -f update_fhir_replay_status.psql
```